...
void setup() {
  ...
  if (width>displayWidth) 
  {
    ...
    println("CANVAS too big, needed to be readjusted to fit on your monitor.");
  } else {
    appWidth=width;
...
void mousePressed() {
  ...
  if ( midLow==true) {
    if (mouseX>=midLowX && mouseX<=midLowX+midLowWidth && mouseY>=midLowY && mouseY<=midLowY+midLowHeight ) { 
      LeftLow=true; 
      picDraw5=true;
    }
  }
  if ( LeftLow==true) {
    if (mouseX>=leftLowX && mouseX<=leftLowX+leftLowWidth && mouseY>=leftLowY && mouseY<=leftLowY+leftLowHeight ) { 
      LeftMid=true; 
      picDraw6=true;
    }
  }
  if ( LeftMid ==true) {
    if (mouseX>=leftMidX && mouseX<=leftMidX+leftMidWidth && mouseY>=leftMidY && mouseY<=leftMidY+leftMidHeight ) { 
      LeftTop=true; 
      picDraw7=true; 
      nightMode=true;
      backgroundColor = color (random(255), random (255), 0 );
  ...
  if (LeftTop==true) {
    if ( mouseX>=leftTopX && mouseX<=leftTopX+leftTopWidth && mouseY>=leftTopY && mouseY<=leftTopY+leftTopHeight ) { 
      MidTop=true; 
      picDraw8=true;
    }
  }
  if ( MidTop==true) {
    if (mouseX>=midTopX && mouseX<=midTopX+midTopWidth && mouseY>=midTopY && mouseY<=midTopY+midTopHeight ) {
      exit();
    }
  }
  //End Hover-Over
